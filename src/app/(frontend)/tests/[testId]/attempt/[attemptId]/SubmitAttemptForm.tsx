...
export default function SubmitAttemptForm({
  testId,
  attemptId,
  questions,
  existingResponses,
  ...
  useEffect(() => {
    ...
    existingResponses.forEach((response) => {
      initialResponses[response.question.id] = response.response
    })
  ...
  const handleApiError = useCallback(async (response: Response) => {
    if (response.status === 401) {
      setError('You need to be logged in to save responses. Please refresh the page and try again.')
      return false
    }
    if (response.status === 403) {
      setError('You do not have permission to save responses for this test.')
      return false
    }
    if (response.status === 400) {
      const data = await response.json()
      setError(data.error || 'There was an issue with your response.')
      return false
    }
  ...
  const saveResponse = async () => {
    ...
    try {
      const verifyResponse = await fetch(`/api/tests/${testId}/attempt/${attemptId}/verify`, {
        method: 'GET',
        headers: {
      ...
      if (!verifyResponse.ok) {
        ...
        setError(verifyData.error || 'Failed to verify attempt')
        setSavingResponse(false)
        return false
      }

      const verifyData = await verifyResponse.json()
      const correctTestId = verifyData.testId

      if (!correctTestId) {
        setError('Failed to verify test ID')
        setSavingResponse(false)
        return false
      }

      const response = await fetch(`/api/tests/${correctTestId}/attempt/${attemptId}/response`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleNext = async () => {
    ...
    if (saved && currentQuestionIndex < questions.length - 1) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/tests/${correctTestId}/attempt/${attemptId}/submit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (
        ...
        setError(`Network error. Retrying submission... (${retryCount + 1}/${MAX_RETRIES})`)
      ...
      <main className="mx-auto mt-6 grid max-w-7xl grid-cols-1 lg:grid-cols-12 gap-6 px-4 md:px-6 pb-12">
        ...
        <aside className="lg:col-span-3 flex flex-col gap-6">
          ...
          >
            ...
            {submittingTest ? 'Submitting...' : 'Finish Attempt'}
          </button>
        </aside>
          ...
          <div className="rounded-xl border border-white/5 bg-slate-900 overflow-hidden">
            <div className="bg-slate-800/50 px-8 py-4 border-b border-white/5 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-semibold text-slate-500">
                  {currentQuestion.type === 'multiple_choice' ? 'Multiple Choice' : currentQuestion.type === 'true_false' ? 'True/False' : 'Short Answer'} • 2 Points
                </span>
              </div>
              <button
            ...
            <div className="bg-slate-800/50 px-8 py-4 border-t border-white/5">
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  onClick={handleNext}
                  disabled={currentQuestionIndex === questions.length - 1 || loading || !responses[currentQuestion.id]}
